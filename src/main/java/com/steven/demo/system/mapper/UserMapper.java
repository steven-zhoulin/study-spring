...
public interface UserMapper {
    ...
    @Insert("insert into user(username, password, phone, email, dept_id, job_id, create_time, enabled) " +
            "values(#{username}, #{password}, #{phone}, #{email}, #{deptId}, #{jobId}, now(), #{enabled})")
    public int add(User user);

    @Delete("delete from user where id = #{id}")
    public int delete(String id);

    @Update("update user set phone=#{phone}, email=#{email}, password=#{password}, dept_id=#{deptId}, " +
            "job_id=#{jobId}, enabled=#{enabled} " +
            "where id=#{id}")
